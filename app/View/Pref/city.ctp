...
<div class="container cf">
	...
	?>
	<section>
		<h1 class="ttl mb10 bg-main-color white pl10"><?= $data_city['name'] ?>に派遣可能なおすすめデリヘル店</h1>
		<?php
	...
	<header class="cf">
		...
		if ($is_dayuse) {
		?>
		<h1 class="ttl mb10 col span-8 bg-gray pl10"><?= $pref_meta['ja'] ?><?= $data_city['name'] ?>のデリヘルを呼べるホテル</h1>
		...
		?>
		<h1 class="ttl mb10 bg-gray pl10"><?= $pref_meta['ja'] ?><?= $data_city['name'] ?>のデリヘルを呼べるホテル</h1>
		<?php
		}
		?>
	</header>
	...
	for ($i=1; $i<=3; $i++) {
		if (isset($hotels[$i])) {
			...
			foreach ($hotels[$i] as $v) {
				print '<tr>';
				print '<td>';
				print '<span class="i-building-o gray"></span>&nbsp;<a href="/hotel/' . $v['id'] . '/">' . $v['name'] . '</a>';
				if($this->common->isSP()){
					if($v['cnt'] != 0){
						print '&nbsp;<div class="tar dib"><span class="i-commenting fs16"></span><span class="counter is-left">' . number_format($v['cnt']) . '</span></div>';
					}
				}
				print '</td>';
				...
				if ($this->common->isSP()){
					?>
					<td class="tac"><a href="https://maps.google.com/maps?saddr=%E7%8F%BE%E5%9C%A8%E5%9C%B0&daddr=<?= urlencode($v['address']) ?>&dirflg=w" class="ls-none" target="_blank" onclick="ga('send', 'event', {'eventCategory':'click', 'eventAction':'Guide', 'eventLabel':'[<?= $v['pref_id'] ?>]<?= $v['name'] ?>]})"><span class="i-michijun-rectangle fs30"></span></a></td>
					<?php
				}
				if(!$this->common->isSP()){
					if($v['cnt'] != 0){
	...
	?>
	<section>
		<h2 class="ttl mb10 bg-main-color white pl10"><?= $data_city['name'] ?>に派遣可能なデリヘル店一覧</h2>
		<?php
